...
QCircuit grover_oracle(uint32_t search_qubits, uint32_t marked_state,
    ...
    if (marked_state >= fast_pow2(search_qubits))
        throw std::invalid_argument{
            "Marked state should be in the range [0, 2^search_qubits)"};

    QCircuit qc{search_qubits};

    for (uint32_t i = 0; i < search_qubits; ++i) {
        if (!(marked_state & (1 << i))) qc << X(i);
    }

    // Generate a N-Control Z gate
    qc << Gate{NControl_Gate(search_qubits, 0, search_qubits - 1,
                             search_qubits - 1, Z::gate()),
               0};
...
QCircuit grover_search(uint32_t search_qubits, const QCircuit& oracle,
    ...
    if (oracle.qubit_count() < search_qubits)
        throw std::invalid_argument{
            "Cannot use given oracle for this qubit circuit"};

    QCircuit qc(oracle.qubit_count());
    for (uint32_t i = 0; i < search_qubits; ++i) qc << H{i};
    ...
    for (uint32_t i = 0; i < iterations; ++i) {
        ...
        for (uint32_t j = 0; j < search_qubits; ++j) qc << H{j};

        // qc << Z{search_qubits - 1};
        for (uint32_t j = 0; j < search_qubits; ++j) qc << X{j};
        // qc << Z{search_qubits - 1};
...
QCircuit grover_iteration(uint32_t search_qubits, const QCircuit& oracle,
                          uint32_t iterations, bool compile) {
    QCircuit qc{search_qubits};
    for (uint32_t i = 0; i < iterations; ++i) {
        qc << Gate{oracle, 0};
        for (uint32_t j = 0; j < search_qubits; ++j) qc << H{j};
...
QCircuit inverse_fourier_transform(uint32_t qubits, bool compile) {
    ...
    for (uint32_t i = 0; i < qubits; ++i) {
        for (int32_t j = i - 1; j >= 0; --j)
            qc << CPhase{static_cast<uint32_t>(j), i,
                         -aqs::pi / (1 << (i - j))};
...
std::vector<std::pair<std::string, af::cfloat>> decompose_hamiltonian(
    ...
    auto hs_product = [](const af::array& lhs, const af::array& rhs) {
        return af::sum<af::cfloat>(
            af::diag(af::matmul(af::transpose(lhs, true), rhs)));
    };
    ...
    for (uint32_t i = 0; i < fast_pow2(qubits * 2); ++i) {
        ...
        auto coeff = af::sum<af::cfloat>(
            af::diag(af::matmul(af::transpose(temp, true), hamiltonian)));
        if ((coeff != af::cfloat{0.f} || coeff != af::cfloat{-0.f}) &&
            (coeff.real * coeff.real + coeff.imag * coeff.imag > 1e-8))
...
af::array compose_hamiltonian(
    ...
    uint32_t qubits = 0;
    if (!description.empty()) qubits = description[0].first.length();

    af::array out =
        af::constant(af::cfloat{}, fast_pow2(qubits), fast_pow2(qubits));
    static af::cfloat ivals[] = {
        {1.f, 0.f}, {0.f, 0.f}, {0.f, 0.f}, {1.f, 0.f}};
    static af::cfloat xvals[] = {
        {0.f, 0.f}, {1.f, 0.f}, {1.f, 0.f}, {0.f, 0.f}};
    ...
    for (const auto& pair : description) {
        ...
        for (char c : str) {
            switch (c) {
                case 'i': temp = tensor_product(temp, I); break;
                case 'x': temp = tensor_product(temp, X); break;
                case 'y': temp = tensor_product(temp, Y); break;
                case 'z': temp = tensor_product(temp, Z); break;
            }
        }
        out += temp * coeff;
    }
...
void add_evolved_pauli_product(aqs::QCircuit& qc, const std::string& shape,
                               float coeff) {
    ...
    for (uint32_t i = 0; i < qc.qubit_count(); ++i) {
        ...
        switch (c) {
            case 'x': qc << aqs::H{i}; break;
            case 'y': qc << aqs::Y{i} << aqs::RotX{i, -aqs::pi / 2.f}; break;
            case 'z': break;
            default: continue;
        }

        if (previous != -1) qc << aqs::CX{previous, i};
    ...
    for (uint32_t i = previous; i > 0; --i) {
        ...
        if (i - 1 != previous) qc << aqs::CX{i - 1, previous};

        switch (shape[previous]) {
            case 'x': qc << aqs::H{previous}; break;
            case 'y':
...
QCircuit linear_entanglement_varstate(uint32_t qubits, uint32_t depth,
    ...
    if (depth == 0)
        throw std::invalid_argument{
            "Depth of circuit must be greater than zero"};
    if (qubits * depth * 2 != values.size())
        throw std::invalid_argument{
            "Parameter count passed is not the expected received"};

    aqs::QCircuit qc{qubits};

    for (uint32_t i = 0; i < qubits; ++i) qc << aqs::RotY{i, values[i]};
    for (uint32_t i = 0; i < qubits; ++i)
        qc << aqs::RotZ{i, values[qubits + i]};

    for (uint32_t j = 0; j < depth - 1; ++j) {
        for (uint32_t i = 0; i < qubits - 1; ++i) qc << aqs::CX{i, i + 1};
...
QCircuit full_entanglement_varstate(uint32_t qubits, uint32_t depth,
    ...
    for (uint32_t j = 0; j < depth - 1; ++j) {
        for (uint32_t i = 0; i < qubits - 1; ++i) {
            for (uint32_t k = i + 1; k < qubits; ++k) qc << aqs::CX{i, k};
        }
...
std::pair<float, std::vector<float>> variational_quantum_eigensolver(
    const af::array& matrix, float range, VQE state_circuit, float tolerance,
    uint32_t max_evaluations) {
    auto dim = matrix.dims();
    if (dim[0] != dim[1] && fast_pow2(fast_log2(dim[0])) != dim[0] &&
        dim[3] != 1 && dim[4] != 1)
    ...
    af::array random_vals =
        (af::randu(params.size(), f64) * af::Pi * 2.0) - af::Pi;
    ...
    auto cost_function = [](const std::vector<double>& x,
                            std::vector<double>& gradient, void* void_data) {
        ...
        switch (data.state) {
            case VQE::LINEAR:
                qc << aqs::Gate{
                    linear_entanglement_varstate(qubits, qubits, param_buff),
                    0};
                break;
            case VQE::FULL:
                qc << aqs::Gate{
                    full_entanglement_varstate(qubits, qubits, param_buff), 0};
                break;
        }
        ...
        af::cfloat expectation =
            af::matmul(bra_state, ket_state)(0).scalar<af::cfloat>();

        double angle = std::acos(expectation.real);
        if (expectation.imag < 0.f) angle = -angle;
