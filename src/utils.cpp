...
static af::array tensor_product_af_cpu(const af::array& mat1,
                                       const af::array& mat2);
static af::array tensor_product_af_opencl(const af::array& mat1,
                                          const af::array& mat2);

af::array tensor_product(const af::array& mat1, const af::array& mat2) {
    if (mat1.type() != mat2.type())
        throw std::invalid_argument{
            "The tensor requires both matrices to have the same type"};
    if (mat1.dims()[2] != 1 || mat1.dims()[3] != 1 || mat2.dims()[2] != 1 ||
        mat2.dims()[3] != 1)
        throw std::invalid_argument{
            "Tensor product only supports two dimensions"};

    switch (af::getActiveBackend()) {
        case AF_BACKEND_CPU: return tensor_product_af_cpu(mat1, mat2);
        case AF_BACKEND_OPENCL: return tensor_product_af_opencl(mat1, mat2);
        default: return tensor_product_af_opencl(mat1, mat2);
    }
}
...
std::string repeat(std::size_t n, const std::string& str) {
    ...
    for (std::size_t i = 0; i < n; i++) out.append(str);
...
af::array insert_bits(const af::array& current, const af::array& insert,
                      ...
                      size_t array_length) {
    if (current.dims()[0] != array_length || insert.dims()[0] != array_length ||
        position.dims()[0] != array_length || offset.dims()[0] != array_length)
        throw af::exception{"Different array lengths"};

    af::array least_mask =
        ((af::constant(1, array_length, s32) << position) - 1);
...
af::array gen_index(uint32_t gate_qubit_begin, uint32_t gate_qubit_count,
    ...
    return insert_bits(
        indices, values,
        af::constant(circuit_qubit_count - gate_qubit_count - gate_qubit_begin,
                     array_length, s32),
...
af::array tensor_product_af_cpu(const af::array& mat1, const af::array& mat2) {
    ...
    auto temp = af::moddims(af::flat(af::tile(af::flat(mat1).T(), cols2)).T(),
                            cols1 * cols2, rows1);

    auto resized_mat1 = af::moddims(af::tile(temp.T(), 1, rows2).T(),
...
af::array tensor_product_af_opencl(const af::array& mat1,
                                   const af::array& mat2) {
    af::dim4 dims1 = mat1.dims();
    af::dim4 dims2 = mat2.dims();

    af::array out = af::tile(mat2, dims1[0], dims1[1]);
    af::array resized_mat1 =
        af::resize(dims2[0], dims2[1], mat1, AF_INTERP_LOWER);
