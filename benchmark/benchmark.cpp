...
void qft_benchmark(uint32_t qubits, uint32_t count)
{
    ...
    std::cout << "QFT\n";
    std::cout << "Total time: " << duration << " ms; Average Time: " << duration / count
            << " ms; Count: " << count << '\n' << std::endl;
}

void grover_benchmark(uint32_t qubits, uint32_t count)
{
    auto start = std::chrono::high_resolution_clock::now();
...
void shor_benchmark(uint32_t count)
{
    auto start = std::chrono::high_resolution_clock::now();
...
int main(int argc, char** argv)
{
    ...
    // aqs::initialize(argc, argv, af::Backend::AF_BACKEND_CPU);

    const uint32_t qubits = 12;
