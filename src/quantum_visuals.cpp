...
void print_statevector(const QSimulator& simulator) {
    ...
    for (int i = 0; i < states - 1; ++i)
        std::cout << vals[i] << "|" << binary_string(i, qubits) << "> + "
                  << ((i + 1) % 4 ? "" : "\n");
...
void print_profile(const std::array<uint32_t, 2>& profile) {
    ...
    std::cout << "|0>: " << profile[0] * 100.f / rep_count << "% ("
              << profile[0] << ")\n"
              << "|1>: " << profile[1] * 100.f / rep_count << "% ("
              << profile[1] << ")\n";
    std::cout << std::setprecision(7) << std::defaultfloat << std::noshowpos;
}
...
void print_profile(const std::vector<uint32_t>& profile) {
    ...
    for (int i = 0; i < profile.size(); i++)
        std::cout << "|" << binary_string(i, qubits) << ">: " << std::setw(5)
                  << profile[i] * 100.f / rep_count << "% (" << profile[i]
                  << ")\n";
...
std::string parse_circuit_representation(std::string parse_string) {
    ...
    // std::regex gate_regex{ "([A-Za-z0-9]+),([0-9]+),([0-9]+):(([0-9]+,?)+);"
    // }; // ASCII ONLY
    std::regex gate_regex{
        "([^,|;]+),([0-9]+),([0-9]+):(([0-9]+,?)+);"};  // UTF8
    ...
    if (std::regex_search(parse_string, info_match, info_regex)) {
        ...
        if (qubit_count > aqs::max_qubit_count)
            throw std::out_of_range{
                "Circuit exceeds supported max qubit count"};
        ...
        while (std::regex_search(initial_states_str, inital_states_match,
                                 initial_states_regex)) {
            // Find the index of qubit state being set
            uint32_t qubit_index =
                static_cast<uint32_t>(std::stoul(inital_states_match[1].str()));

            if (initialized_qubits.count(qubit_index))
                throw std::invalid_argument{
                    "Cannot set the state of the same qubit multiple times"};

            initialized_qubits.insert(qubit_index);

            // Store the state the qubit is in
            auto state =
                static_cast<bool>(std::stoul(inital_states_match[2].str()));
    ...
    auto fill_circuit =
        [&lines, &lines_length, qubit_count, &current_circuit_length](
            std::string center = "", std::string sides = "",
            std::size_t tail_len = 0) {
            for (uint32_t i = 0; i < qubit_count; ++i) {
                lines[i * 3]
                    << repeat(current_circuit_length - lines_length[i], " ")
                    << sides;
                lines[i * 3 + 1]
                    << repeat(current_circuit_length - lines_length[i], "─")
                    << center;
    ...
    auto fill_qubit_range = [&lines, &lines_length, qubit_count,
                             &current_circuit_length](uint32_t first_qubit,
                                                      uint32_t last_qubit) {
        std::size_t max_line_length = 0;

        for (std::size_t i = first_qubit; i <= last_qubit; ++i)
            max_line_length =
                (lines_length[i] > max_line_length ? lines_length[i]
                                                   : max_line_length);
        ...
        current_circuit_length =
            (current_circuit_length < max_line_length ? max_line_length
                                                      : current_circuit_length);
    };
    ...
    while (!parse_string.empty()) {
        ...
        if (std::regex_search(parse_string, gate_match, barrier_regex) &&
            gate_match.prefix().str().empty()) {
            if (gate_match.str() == "B;")
                fill_circuit("▒─", "▒ ", 2);
            ...
            uint32_t control_qubit_count =
                static_cast<uint32_t>(std::stoul(gate_match[2]));
            uint32_t target_qubit_count =
                static_cast<uint32_t>(std::stoul(gate_match[3]));
            std::string qubit_list_str = gate_match[4];
            std::smatch qubit_list_match{};

            while (std::regex_search(qubit_list_str, qubit_list_match,
                                     qubit_list_regex)) {
                uint32_t qubit =
                    static_cast<uint32_t>(std::stoul(qubit_list_match[1]));

                if (qubit >= qubit_count)
                    throw std::out_of_range{
                        "Qubit index is outside the circuit dimensions"};

                if (qubit_set.count(qubit))
                    throw std::invalid_argument{
                        "Invalid gate registers: cannot have the same qubit as "
                        "multiple gate registers"};
            ...
            if (control_qubit_buffer.size() != control_qubit_count ||
                ...
                throw std::invalid_argument{
                    "Number of control and target qubits must match gate "
                    "declaration"};

            // General Constraints
            if (!target_qubit_buffer.size())
                throw std::invalid_argument{
                    "Gate must contain at least one target qubit"};

            // Special Gate Contraints
            if (gate_name == "Swap" && target_qubit_buffer.size() != 2)
                throw std::invalid_argument{
                    "Number of target qubits for a Swap gate must be 2"};
            ...
            const uint32_t first_qubit = [&]() {
                if (!control_qubit_buffer.empty()) {
                    is_control_first = target_qubit_buffer.front() >
                                       control_qubit_buffer.front();
                    return is_control_first ? control_qubit_buffer.front()
                                            : target_qubit_buffer.front();
                } else {
            ...
            const uint32_t last_qubit = [&]() {
                if (!control_qubit_buffer.empty()) {
                    is_control_last = target_qubit_buffer.back() <
                                      control_qubit_buffer.back();
                    return is_control_last ? control_qubit_buffer.back()
                                           : target_qubit_buffer.back();
                } else {
            ...
            while (current_ctrl_index < control_qubit_buffer.size() ||
                   current_trgt_index < target_qubit_buffer.size()) {
                ...
                if (current_ctrl_index < control_qubit_buffer.size()) {
                    if (current_trgt_index < target_qubit_buffer.size()) {
                        current_qubit =
                            target_qubit_buffer[current_trgt_index] <
                                    control_qubit_buffer[current_ctrl_index]
                                ? target_qubit_buffer[current_trgt_index]
                                : control_qubit_buffer[current_ctrl_index];
                    } else {
                        current_qubit =
                ...
                for (uint32_t i = previous_qubit + 1; i < current_qubit; ++i) {
                    lines[i * 3] << repeat(mid, " ") << "│"
                                 << repeat(len - mid - 1, " ");
                    lines[i * 3 + 1] << repeat(mid, "─") << "┼"
                                     << repeat(len - mid - 1, "─");
                    lines[i * 3 + 2] << repeat(mid, " ") << "│"
                                     << repeat(len - mid - 1, " ");
                ...
                if (current_trgt_index < target_qubit_buffer.size() &&
                    ...
                    // Handle general gates
                        ...
                        // Find if there are multiple multi-qubit target qubit
                        // gates
                        for (uint32_t i = current_trgt_index + 1;
                             i < target_qubit_buffer.size() &&
                             target_qubit_buffer[i] ==
                                 target_qubit_buffer[i - 1] + 1;
                             ++i)
                            ++count;
                        ...
                        // Top qubit marks
                        if (!is_control_first &&
                            current_qubit == target_qubit_buffer.front())
                        ...
                        // Mid qubit marks
                        for (uint32_t i = begin + 1; i < begin + count - 1;
                             ++i) {
                        ...
                        if (count > 1) {
                            lines[(begin)*3 + 2] << "  │" << repeat(strlen, " ")
                                                 << "│  ";
                            lines[(begin + count - 1) * 3]
                                << "  │" << repeat(strlen, " ") << "│  ";
                            lines[(begin + count - 1) * 3 + 1]
                                << "──┤" << repeat(strlen, " ") << "├──";
                        }
                        if (!is_control_last && current_qubit + count - 1 ==
                                                    target_qubit_buffer.back())
                            lines[(begin + count - 1) * 3 + 2]
                                << "  └" << repeat(strlen, "─") << "┘  ";
                        else
                            lines[(begin + count - 1) * 3 + 2]
                                << "  └" << repeat(strmid, "─") << "┬"
                    ...
                    if (is_control_first &&
                        current_qubit == control_qubit_buffer.front()) {
                        lines[current_qubit * 3.] << repeat(len, " ");
                        lines[current_qubit * 3 + 1]
                            << repeat(mid, "─") << "█"
                            << repeat(len - mid - 1, "─");
                        lines[current_qubit * 3 + 2]
                            << repeat(mid, " ") << "│"
                            << repeat(len - mid - 1, " ");
                    }
                    // If control qubit is the last qubit in the gate
                    else if (is_control_last &&
                             current_qubit == control_qubit_buffer.back()) {
                        lines[current_qubit * 3] << repeat(mid, " ") << "│"
                                                 << repeat(len - mid - 1, " ");
                        lines[current_qubit * 3 + 1]
                            << repeat(mid, "─") << "█"
                            << repeat(len - mid - 1, "─");
            ...
            throw std::invalid_argument{
                "Could not parse the given circuit string representation"};
        }
    ...
    for (const auto& line : lines) output.append(line.str());
...
std::string gen_circuit_text_image(const QCircuit& circuit,
    ...
    if (circuit.qubit_count() != simulator.qubit_count())
        throw std::invalid_argument{
            "Circuit and simulator qubit count must match"};
    ...
    for (int i = 0; i < qubits; ++i) {
        ...
        if (q == aqs::QState::zero())
            ...
            throw std::invalid_argument{
                "Superposed inital states not supported"};
