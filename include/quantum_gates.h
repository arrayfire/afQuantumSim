...
{
    /**
     * @brief Creates circuit containing the same gate added at multiple target qubits
     * 
     ...
     */
    QCircuit Group_Gate(uint32_t qubits, std::vector<uint32_t> target_qubits, QCircuit gate, bool compile = false);
     ...
     */
    QCircuit NControl_Gate(uint32_t qubits, uint32_t control_qubit_begin, uint32_t control_qubit_count,
                           uint32_t target_qubit_begin, const QCircuit& gate, bool compile = false);

    /**
     * @brief Creates a circuit containing a gate controlled by the given number of qubits
     * 
     * @param qubits number of qubits in the circuit
    ...
    /**
     * @brief Creates a circuit where the connections have been rewired for the qubits
     * 
     * @note the new qubit positions must map every single qubit in the gate to a different qubit desired in the circuit
     * 
     * @param qubits number of qubits in the circuit
     ...
     */
    QCircuit Rewire_Gate(uint32_t qubits, const std::vector<uint32_t>& new_qubit_positions, const QCircuit& gate,
                           bool compile = false);
