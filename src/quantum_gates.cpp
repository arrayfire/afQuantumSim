...
QCircuit Group_Gate(uint32_t qubits, std::vector<uint32_t> target_qubits,
    ...
    if (target_qubits.size() > qubits)
        throw std::invalid_argument{
            "Cannot add more target qubits than there are total qubits"};

    gate.compile();
    QCircuit qc{qubits};

    for (const auto& qubit : target_qubits) qc << Gate{gate, qubit};
...
QCircuit Control_Group_Gate(uint32_t qubits, uint32_t control_qubit,
    ...
    if (target_qubits.size() >= qubits)
        throw std::invalid_argument{
            "Cannot add control gate at the given position"};

    auto pivot = std::lower_bound(target_qubits.cbegin(), target_qubits.cend(),
                                  control_qubit);
    if (*pivot == control_qubit)
        throw std::invalid_argument{
            "Cannot add control gate at the target qubit positions"};
...
QCircuit NControl_Gate(uint32_t qubits, uint32_t control_qubit_begin,
    ...
    if (control_qubit_count == 0)
        throw std::invalid_argument{
            "The number of control qubits must be at least 1"};
    ...
    if (control_qubit_count == 1) {
        ...
        QCircuit temp(gate.qubit_count() + 1 + target_qubit_begin -
                      control_qubit_begin - control_qubit_count);
        temp << ControlGate(
            gate, 0,
            target_qubit_begin - control_qubit_begin - control_qubit_count + 1);
...
QCircuit NControl_Gate(uint32_t qubits, std::vector<uint32_t> control_qubits,
    ...
    if (control_qubits.size() == 0)
        throw std::invalid_argument{
            "Number of control qubits must be at least one"};
    if (gate.qubit_count() + control_qubits.size() > qubits)
        throw std::invalid_argument{
            "Invalid number of qubits for number of control of qubits and gate "
            "count"};
    if (target_qubit_begin + gate.qubit_count() > qubits)
    ...
    auto pivot = std::lower_bound(control_qubits.begin(), control_qubits.end(),
                                  target_qubit_begin);
    if (pivot != control_qubits.end() &&
        *pivot < target_qubit_begin + gate.qubit_count())
        throw std::invalid_argument{
            "Cannot add control gate at the target qubit positions"};
    ...
    if (top.size() != 0) {
        ...
        temp << ControlGate(current, top.front(),
                            qubits - current.qubit_count());
        current = std::move(temp);
    }

    if (compile) current.compile();
...
QCircuit Rewire_Gate(uint32_t qubits,
    ...
    if (new_qubit_positions.size() != gate.qubit_count())
        throw std::invalid_argument{
            "New qubit positions must map all the qubits in the gate"};
    if (gate.qubit_count() > qubits)
        throw std::domain_error{
            "Cannot rewire circuit to a lower number of qubits"};

    std::vector<uint32_t> sort_positions = new_qubit_positions;
    std::partial_sort_copy(new_qubit_positions.begin(),
                           new_qubit_positions.end(), sort_positions.begin(),
                           sort_positions.end());
    for (uint32_t i = 1; i < sort_positions.size(); ++i) {
        if (sort_positions[i - 1] == sort_positions[i])
            throw std::invalid_argument{
                "Cannot rewire multiple qubits to the same qubit"};
    }
    ...
    for (uint32_t i = 0; i < new_qubit_positions.size(); ++i) {
        if (swapped.find(i) != swapped.end()) continue;
        swapped.insert(i);
    ...
    for (auto& g_ptr : list) {
        switch (g_ptr->type()) {
            ...
            case Gate::static_type(): {
                ...
                g.internal_circuit = std::make_shared<QCircuit>(
                    Adjoint_Gate(*g.internal_circuit));
                break;
            }
            ...
            default:
                throw std::runtime_error{
                    "Unknown unsupported gate cannot be adjoint"};
        }
    }
